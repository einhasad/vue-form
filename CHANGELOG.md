...
- Optional Ant Design Vue adapter at the `./ant-design` subpath — a single `FormField` scoped-slot wrapper around `<a-form-item>` + `useField`. The main entry stays headless; the adapter is only pulled in if you import from `@einhasad/vue-form/ant-design`.
...
- Vite library mode, three entries: main (`@einhasad/vue-form`), headless (`@einhasad/vue-form/headless`), Ant Design adapter (`@einhasad/vue-form/ant-design`).
