...
import { vehicleApi, validationApi, partLookupApi, type ServerRule } from '../services'
import { useEventLog } from '../composables/useEventLog'
import EventConsole from '../components/EventConsole.vue'

const { events, log, clear: clearLog } = useEventLog()

// Headless form state — provides the form context to descendant useField calls.
...
const data = ref({
  ...
  primaryPart: null as { id: number | string; name: string } | null,
...
// --- Submit orchestration (replacing what <Form> used to do) -----------
async function onSubmit() {
  ...
  try {
    ...
    log('submit', 'vehicleApi.create called', payload)
    const result = await vehicleApi.create(payload)
    submitted.value = result
    log('success', 'form submitted', result)
  } catch (raw) {
    onFail(raw)
  } finally {
    loading.value = false
...
function onFail(raw: unknown) {
  const r = raw as { response?: { status?: number; data?: { result?: { field: string; message: string }[] } }; message?: string }
  if (r?.response?.status === 422 && Array.isArray(r.response.data?.result)) {
    log('fail', `422 — ${r.response.data!.result.length} field error(s)`, r.response.data)
  ...
  log('fail', 'submit failed', raw)
  formCtx.setFormErrors([
    { message: typeof raw === 'string' ? raw : (r?.message ?? 'Submit failed') },
  ])
}
...
// --- Parts array (replacing what <NestedForm> used to do) --------------
...
onMounted(async () => {
  ...
  log('info', `loaded validation schema (${Object.keys(schema).length} key(s))`, schema)
  ...
  const vinUniqueOnServer: Validator = async (v) => {
    ...
    if (taken) log('validator', `vin "${v}" reported taken by server`)
    ...
    <fieldset :disabled="loading" class="fields">
      ...
      >
        <a-input
          v-model:value="field.value"
          :status="field.status"
          placeholder="Try `reject` for a 422 response, `crash` for a network error"
          @blur="field.onBlur"
        />
      </FormField>
      ...
      <h3 class="parts-heading">Parts</h3>
          ...
          <tr v-for="(row, index) in data.parts" :key="index">
            <td>
              <!-- required / minLen / maxLen come from "parts.*.sku" pattern.
                   Inline validators below are client-only. -->
              <FormField
                v-model="row.sku"
...
.result { background: #f9f9f9; padding: 0.75rem; border-radius: 4px; margin-top: 1rem; font-size: 0.85rem; overflow: auto; }
</style>
